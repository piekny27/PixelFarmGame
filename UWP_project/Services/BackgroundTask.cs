...
namespace UWP_project.Services
{
	public sealed class BackgroundTask : IBackgroundTask
	{
		...
		public async void Run(IBackgroundTaskInstance taskInstance)
		{
			...
			Log.debug(this, "Test of logging from async task");
		...
		public static BackgroundTaskRegistration ScheduleOneTime(string taskName, uint timeInMinutes)
		{
			...
			{
				Log.info(TAG, "The task " + taskName + " was already scheduled.");
				return null;
			}
			...
			int i = 0;
			Log.info(TAG, "There are now " + tasks.Count.ToString() + " tasks scheduled:");
			foreach (var task in tasks)
			{
				Log.info(TAG, (i++ + 1).ToString() + " (ID " + task.Value.TaskId + ") = " + task.Value.Name);
			}
